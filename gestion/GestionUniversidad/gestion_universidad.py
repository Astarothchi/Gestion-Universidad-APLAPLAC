...
sedes = ["Concepcion", "Coronel"]# Sedes de la universidad

def cargar_sedes():
    """Carga las sedes desde un archivo JSON.""" 
    try:
        with open("gestion/db_sys/sedes.json", "r") as file:
            data = json.load(file)
            sedes.extend(data)
...
def guardar_sedes():
    ...
    with open("gestion/db_sys/sedes.json", "w") as file:
...
def registrar_eliminacion_sede(sede):
    ...
    try:
        with open("gestion/db_sys/registro_eliminaciones.json", "r") as file:
            eliminaciones = json.load(file)
    except FileNotFoundError:
        eliminaciones = []

    eliminaciones.append({"tipo": "sede", "nombre": sede})
    with open("gestion/db_sys/registro_eliminaciones.json", "w") as file:
